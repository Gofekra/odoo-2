...
class ProjectIssueStage(models.Model):
    ...
    doublon_allowed = fields.Boolean(string='Doublon allowed', help='If this stage can contain many tasks having the same technical name',
        translated=True, default=True)
    parent_stage = fields.Many2one('project.issue.stage', string='Precedent stage', translated=True)
    user_ids = fields.Many2many('res.users', string='Responsible users', translated=True)
    related_fields = fields.Many2many('ir.model.fields', string='Related fields', translated=True, domain=[('model_id.model','=','project.issue')])
