...
class UfRptStage(models.Model):
    ...
    #self.env['ir.sequence'].get('uf.rpt.stage') or '1000')
    ...
    #line_ids = fields.One2many('uf.rpt.stage.line', 'tpl_id', 'Lines')
    ...
    #     print "ufrptstagevals:", vals
    #     ufrptstage_id = super(UfRptStage, self).create(vals)
    #     return ufrptstage_id
    ...
    def do_print(self, cr, uid, ids, context=None):
        ...
        return self.pool['report'].get_action(cr, uid, ids, 'ct_project_uf.uf_rpt_stage_report', context=context)

    @api.multi
    def do_preview(self, context=None):
        # assert len(self.ids) == 1, 'This option should only be used for a single id at a time'
        return {
            'name': 'action_uf_rpt_stage_report',
            'type': 'ir.actions.act_url',
            'url': '/report/html/ct_project_uf.uf_rpt_stage_report/'+str(self.ids[0]),
            'target': 'new',
        }
