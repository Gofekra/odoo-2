...
class UfSoft(models.Model):
    ...
    #self.env['ir.sequence'].get('uf.soft') or '1000')
    ...
    #line_ids = fields.One2many('uf.soft.line', 'tpl_id', 'Lines')
    ...
    #     print "ufsoftvals:", vals
    #     ufsoft_id = super(UfSoft, self).create(vals)
    #     return ufsoft_id
    ...
    def do_print(self, cr, uid, ids, context=None):
        ...
        return self.pool['report'].get_action(cr, uid, ids, 'ct_project_uf.uf_soft_report', context=context)

    @api.multi
    def do_preview(self, context=None):
        # assert len(self.ids) == 1, 'This option should only be used for a single id at a time'
        return {
            'name': 'action_uf_soft_report',
            'type': 'ir.actions.act_url',
            'url': '/report/html/ct_project_uf.uf_soft_report/'+str(self.ids[0]),
            'target': 'new',
        }
