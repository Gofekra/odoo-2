...
class UfDataDynamic(models.Model):
    ...
    #self.env['ir.sequence'].get('uf.data.dynamic') or '1000')
    ...
    #line_ids = fields.One2many('uf.data.dynamic.line', 'tpl_id', 'Lines')
    ...
    #     print "ufdatadynamicvals:", vals
    #     ufdatadynamic_id = super(UfDataDynamic, self).create(vals)
    #     return ufdatadynamic_id
    ...
    def do_print(self, cr, uid, ids, context=None):
        ...
        return self.pool['report'].get_action(cr, uid, ids, 'ct_project_uf.uf_data_dynamic_report', context=context)

    @api.multi
    def do_preview(self, context=None):
        # assert len(self.ids) == 1, 'This option should only be used for a single id at a time'
        return {
            'name': 'action_uf_data_dynamic_report',
            'type': 'ir.actions.act_url',
            'url': '/report/html/ct_project_uf.uf_data_dynamic_report/'+str(self.ids[0]),
            'target': 'new',
        }
