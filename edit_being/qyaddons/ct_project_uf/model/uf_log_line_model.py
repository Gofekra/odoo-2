...
class UfLogLine(models.Model):
    ...
    #self.env['ir.sequence'].get('uf.log.line') or '1000')
    ...
    #line_ids = fields.One2many('uf.log.line.line', 'tpl_id', 'Lines')
    ...
    def do_print(self, cr, uid, ids, context=None):
        ...
        return self.pool['report'].get_action(cr, uid, ids, 'ct_project_uf.uf_log_line_report', context=context)

    @api.multi
    def do_preview(self, context=None):
        # assert len(self.ids) == 1, 'This option should only be used for a single id at a time'
        return {
            'name': 'action_uf_log_line_report',
            'type': 'ir.actions.act_url',
            'url': '/report/html/ct_project_uf.uf_log_line_report/'+str(self.ids[0]),
            'target': 'new',
        }
