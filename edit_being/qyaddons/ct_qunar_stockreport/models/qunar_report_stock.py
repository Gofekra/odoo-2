...
class qunar_report_stock_wizard(models.Model):
	...
	date_stop = fields.Date('Date Stop',default=datetime.strftime(datetime.now(),'%Y-%m-%d'))
	...
	def btn_query(self):
		...
		tools.drop_view_if_exists(self._cr,'qunar_report_stock_line')
...
from ir_property where name='standard_price') ppl on pt.id = ppl.t_id
		...
		res = {
			'name':_('Stock Report ')+ (date_start+'-'+date_stop),
...
class qunar_report_stock_line(models.Model):
	...
	uom=fields.Many2one('product.uom',related="product_id.uom_id",string='Unit')
	last_remainder=fields.Float(string='Init Qty')
	last_amt=fields.Float(string='Init Amount')
	in_stock_qty=fields.Float(string='In Stock Quantity')
	in_stock_amt=fields.Float(string='In Stock Amount')
	out_stock_qty=fields.Float(string='Out Stock Quantity')
	out_stock_amt=fields.Float(string='Out Stock Amount')
	inventory_short_qty=fields.Float(string='Inventory Short Quantity')
	inventory_short_amt=fields.Float(string='Inventory Short Amount')
	inventory_over_qty=fields.Float(string='Inventory Over Quantity')
	inventory_over_amt=fields.Float(string='Inventory Over Amount')
	remainder_qty=fields.Float(string='Remainder Qty')
	remainder_amt=fields.Float(string='Remainder Amount')
