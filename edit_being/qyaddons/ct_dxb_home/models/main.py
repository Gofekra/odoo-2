...
class Theme_switch(http.Controller):
    ...
    @http.route(['/theme_switch/theme_customize'], type='json', auth="public", web=True)
    def theme_customize(self, enable, disable, get_bundle=False):
        ...
        if get_bundle:
            ...
            return request.env["ir.qweb"].sudo()._get_asset('web.assets_backend', options=context)
        return True


    @http.route(['/theme_switch/theme_customize_get'], type='json', auth="public", web=True)
    def theme_customize_get(self, xml_ids):
        enable = []
        ids = self.get_view_ids(xml_ids)
        for view in request.env['ir.ui.view'].sudo().with_context(active_test=True).browse(ids):
            if view.active:
