...
var ShowPaymentLineWidget = form_common.AbstractField.extend({
    render_value: function() {
        ...
        if (info !== false) {
            ...
            var self = this;
            if(invoice_id){
             new Model("account.invoice").query(["origin"]).filter([["id","=",invoice_id]]).first().then(function(result){
               new Model("sale.order").query(["slaes_type"]).filter([["name","=",result.origin]]).first().then(function(data){
                   if(data.slaes_type!='local'){
                        self.$('.outstanding_credit_assign').click();
                   }
               })
             });
            }
