...
class wxcorp_config_settings(models.TransientModel):
    ...
    Corp_Agent = fields.Char('Corp Agent ID', default='4')
    Corp_AccessToken = fields.Char('当前Corp_AccessToken', readonly=True)

    Corp_Url = fields.Char('Corp_Url', readonly=True)
    Corp_Token = fields.Char('Corp_Token', default='LtdtestToken')
    Corp_AESKey = fields.Char('Corp_AESKey', default='hbDSRXhiKkPJJLLwBegeOW9yfxjV7D4q2rCIsRrlPff')
    ...
    def get_default_Corp_Id(self, fields):
        Param = self.env["ir.config_parameter"]
        return {
                'Corp_Id': Param.get_param('Corp_Id', default='wxc169dbf2e6da7c19'),
                'Corp_Secret': Param.get_param('Corp_Secret', default='hPtWksekWPVTYId2KyC6dh0zKSEV2Tmm8'),
                'Corp_Agent': Param.get_param('Corp_Agent', default='4'),
                'Corp_Token': Param.get_param('Corp_Token', default='LtdtestToken'),
                'Corp_AESKey': Param.get_param('Corp_AESKey', default='esGH2pMM98SwPMMQpXPG5Y5QawuL67E2aBvNP10V8Gl'),
                'Corp_Channel': int(Param.get_param('Corp_Channel', default=1)),
                }
