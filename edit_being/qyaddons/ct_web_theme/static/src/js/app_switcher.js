...
var webhome = require('web_home_theme');
var BackEndMenu = require('web_enterprise.BackEndMenu');
var QWeb = core.qweb;
var NBR_ICONS = 6;
...
var AppSwitcher = Widget.extend({
    ...
    appath:"/ct_web_theme/static/src/icon/",
    matchMedia:function(){
        if(window.matchMedia){
            var self = this,
            result = window.matchMedia("(orientation:portrait)");
            if(result.matches){
                return true;
            }else{
                return false;
    ...
    init: function (parent, menu_data) {
        ...
        this.child_menu_data = menu_data;
    ...
    fixFooter:function(){
        //新增代码
        var clone = $("<div>",{class:"Footer_clone"}).append(this.$('#AppSwitcher-swiper').clone().addClass('con'));

        $("body").append(clone);

       new BackEndMenu().load(clone);

        clone.find('.o_app').click(function(event){

            var local = window.location;

            window.location.replace(local.origin+local.pathname+local.search+$(this).attr('href'));
        ...
        visit(menu_data, function (menu_item, parents) {
            ...
            if (!menu_item.parent_id) {
                if (menu_item.web_icon_data) {
                    var split = item.web_icon.split(","),
                    cite = split[0]+"_"+split[1].match(/(\w+)\.(\w+)$/i)[1];
                    if(_.includes(isImgData,cite)){
                        item.web_icon_data = cite;
                        item.icon_cust = true;
                    }else{
    ...
    render: function() {
        ...
        new BackEndMenu().load(this.$main_content);

        this.$menu_search.find("#Dropdown").html(QWeb.render('AppSwitcher.Dropdown', { widget: this }));
        if(this.$el.find(".cchong.home").length<=0){

            webhome.include({
                template:"AppSwitcher.WebHome"
            });

            var home = new webhome(this,this.child_menu_data);
            
            home.insertBefore(this.$main_content);
        }
...
// odoo.define('ct_web.ExpirationPanel', function (require) {
// "use strict";
...
// var AppSwitcher = require('ct_web.AppSwitcher');
